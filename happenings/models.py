...
class Event(models.Model):
    ...
    attending       = models.ManyToManyField(UserModel, blank=True, null=True)
    ...
    def get_absolute_url(self):
        if self.featured:
            return ('special_event_detail', [self.slug])
        return ('cal_event_detail', [str(self.id)])
    ...
    def save(self):
        self.geocode = get_geocode(self.city, self.state, self.address, self.zipcode)
    ...
    def get_top_assets(self):
        images = self.get_all_images()[0:12]
